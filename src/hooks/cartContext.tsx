...
export const CartProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchCart = async () => {
    try {
      ...
      try {
        const auth = await MainApiRequest.get<{ data: { phoneCustomer?: string; phone?: string } }>(
          '/auth/callback'
        );
  ...
  const addToCart = async (
    ...
    await MainApiRequest.post('/cart', payload).catch((err) => {
  ...
  const updateItem = async (
    ...
    try {
      ...
      try {
        const auth = await MainApiRequest.get<{ data: { phoneCustomer: string } }>(
          '/auth/callback'
        );
  ...
  const clearCart = async () => {
    ...
    try {
      ...
      await fetchCart();
      console.log(
        'Done clear, just fetched by',
        usedPhoneCustomer ? 'phoneCustomer=' + usedPhoneCustomer : 'sessionId=' + sid
      );
    } catch (err) {
      console.error('Clear cart failed', err);
      throw err;
