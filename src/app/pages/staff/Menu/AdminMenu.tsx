...
const AdminMenu = () => {
  ...
  const handleApplyCoupon = async () => {
    try {
      ...
      if (phone) {
        const customerResponse = await AdminApiRequest.get(
          `/customer/${phone}`
        );
        const customer = customerResponse.data;

        if (customer?.rank) {
          const membershipResponse = await AdminApiRequest.get(
            `/membership/${customer.rank}`
          );
  ...
  const handlePayment = async () => {
    try {
      ...
      if (Object.keys(order).length === 0 || totalPrice === 0) {
        message.warning(
          "Đơn hàng không có sản phẩm hoặc tổng tiền không hợp lệ."
        );
        return;
      }
            ...
            <div className="invoice-info">
              ...
              <div className="info-item">
                ...
                <span className="value">
                  {orderInfo?.serviceType === "Take Away"
                    ? "Mang đi"
                    : `Bàn ${orderInfo?.tableID || "2"}`}
                </span>
              </div>
            </div>
          </div>
                        ...
                        <div className="item-controls">
                          <div className="quantity-controls">
                            <Button
                              size="small"
                              onClick={() =>
                                updateQuantity(productKey, quantity - 1)
                              }
                            >
                              -
                            </Button>
